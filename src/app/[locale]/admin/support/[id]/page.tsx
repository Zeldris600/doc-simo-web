...
export default function AdminThreadPage() {
  ...
  const sendMessageMutation = useSendSupportMessage(threadId, {
    ...
    onError: () => {
      toast.error("Failed to transmit synchronization.");
    }
  });

  // Flat list of messages from infinite query pages
  ...
  useEffect(() => {
    ...
    channel.bind("support.message.new", (newMessage: SupportMessage) => {
      ...
      queryClient.setQueryData<InfiniteData<MessagesResponse>>(["support-messages", threadId], (oldData) => {
        ...
        // Check if message already exists (to prevent double-entry if mutation also adds it)
      ...
      // Still invalidate to ensure backend/frontend state is fully synced (pagination, status, etc.)
      queryClient.invalidateQueries({ queryKey: ["support-messages", threadId] });
      
      if (newMessage.senderUserId !== user?.id) {
          toast.info(`New signal from ${newMessage.senderRole}`);
      }
    });
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between p-4 bg-white border-b border-gray-100 rounded-t-xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Active Consulting Session</p>
          </div>
        </div>
      </div>
      ...
      >
        {hasNextPage && (
          <div className="flex justify-center">
            ...
            >
              {isFetchingNextPage ? "Syncing..." : "Load Older Signals"}
            </Button>
          </div>
        )}
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center opacity-20">
            ...
            <p className="text-xs font-black uppercase tracking-widest">No signals recorded in this thread</p>
          </div>
        )}
      ...
      >
        <Input
          ...
          placeholder="Transmit a message to the customer..."
        ...
        <Button 
          ...
          className="h-12 w-12 rounded-xl bg-black hover:bg-black/90 text-white shrink-0 p-0"
        >
